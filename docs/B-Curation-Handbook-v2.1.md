...
**Companion** *A · Build PRD v2.1* — owns screens, schema and validation mechanism

### Changelog 2.0 → 2.1

| # | Change |
|---|---|
| 1 | The UK joins Germany as the second reviewed country package: 50 records at `/uk/` |
| 2 | A verified Chinese title is no longer mandatory; `title_zh` may be null and the page uses `title_original` |
| 3 | The sub-tag range is now 1–4, preventing fabricated second tags |
| 4 | `poetry` and `original_only` are added to the controlled vocabulary |
| 5 | Country-specific regions, periods, sub-tags, sources and quotas are isolated per destination |
...
|---|---|
| Total | **50** |
| 影像 | 21（电影 11 · 剧集 8 · 纪录片 2） |
| 书籍 | 18 |
| 音乐 | 11 |
| 梯队 | 必读之选 25 · 余力之选 17 · 归来之后 8 |
| 创作视角 | 本地 39 · 外来 7 · 移民与跨文化 4 |
| `region_primary` | 伦敦 17 · 苏格兰 8 · 英格兰南部与西南 8 · 英格兰北部 6 · 全国 4 · 北爱尔兰 3 · 英格兰中部 2 · 威尔士 2 |
| 子标签 | 15 个全部有覆盖 |
| `cn_edition` | 有中译本 20 · 有字幕 16 · 无语言门槛 13 · 仅原文 1 |

The UK package is approved to publish at 50 records while its configured targets remain visible. The quota report is printed during every build and does not block until the curator explicitly enables enforcement; this is a recorded release exception, not a relaxation of the targets.
...
1–4 per work. One precise tag is preferable to a fabricated second tag. Colour comes from the sub-tag's own home theme. **A record's sub-tags need not be covered by its `theme_main`** — see Build PRD §8.5.
...
| Medium | Requirement |
|---|---|
| 书籍 | A published Chinese translation, a work originally written in Chinese, or an explicitly approved `original_only` exception |
...
**Germany currently unmet:** 移民与跨文化视角 (2/4) and 萨克森 primary (1/2).

**United Kingdom currently unmet:** total (50/58), 移民与跨文化视角 (4/10), 历史与记忆 (7/8), 北爱尔兰 primary (3/4), 英格兰中部 primary (2/3), 威尔士 primary (2/3), and 一个晚上 (15/16).

Both country packages currently have an explicit release exception: schema errors still fail every build, every quota gap is printed in development and production, and quota enforcement can be re-enabled only by changing the country's configuration after the gaps are resolved.
