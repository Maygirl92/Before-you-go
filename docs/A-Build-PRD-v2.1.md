...
**Brief panel contents:** destination name, non-empty intro fields from that destination's config, the five theme chips, and a button using the pattern `进入{国家} · {数量}部作品`.
